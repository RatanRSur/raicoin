...
class RootBlock(
  val ledger: Ledger = new Ledger()) extends Block {
  val isValid = true
...
class MinedBlock(
  ...
  val miner: String,
  val newUsers: Seq[String]) extends Block {
  ...
  val hashDependencies = Seq[SHAHashable](previousBlock, transactions, ledger, timestamp).map(_.hash)
