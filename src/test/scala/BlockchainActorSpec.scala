...
class BlockchainActorSpec extends FunSuiteLike {
  ...
  test("can load blockchain using actor constructor") {
    ...
    try {
      ...
      p.expectMsg(Saved(savePath.toFile.getName))
      val loadingActor = system.actorOf(Props(BlockchainActor.fromBlockchainFile(savePath.toFile)))
  ...
  test("can save \"big\" blockchain") {
    ...
    val blockchainActor =
      system.actorOf(Props(new BlockchainActor(new Blockchain(difficulty = 1))(noBindConfig)))
